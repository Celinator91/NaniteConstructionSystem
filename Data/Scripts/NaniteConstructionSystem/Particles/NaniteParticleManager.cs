...
namespace Ntech.Nanite.Particles
{
    public class NaniteParticleManager
    {
        ...
        private Entities.LargeControlFacilityLogic m_constructionBlock;
        public NaniteParticleManager(Entities.LargeControlFacilityLogic block)
        {
        ...
        public void AddParticle(Vector4 startColor, Vector4 endColor, float minTime, float distanceDivisor, object target, IMyTerminalBlock miningHammer = null)
        {
            ...
            double distance = Vector3D.Distance(m_constructionBlock.Entity.GetPosition(), targetPosition);
            int time = (int)Math.Max(minTime, (distance / distanceDivisor) * 1000f);
            int tailLength = Math.Max(1, 15 - ((int)(m_particles.Count / 40f)));
            NaniteParticle particle = new NaniteParticle(time, (IMyCubeBlock)m_constructionBlock.Entity, target, startColor, endColor, tailLength, 0.05f);
