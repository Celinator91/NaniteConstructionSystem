...
namespace Ntech.Nanite.Entities.Effects
{
    ...
    {
        private List<LightningBolt.LightningBoltInstance> m_activeBolts;
        ...
        public CenterOrbEffect(MyCubeBlock block)
        {
            ...
            m_activeBolts = new List<LightningBolt.LightningBoltInstance>();
        }
        ...
        private void UpdateBolts(bool active = false)
        {
            ...
            if (m_activeBolts.Count < m_maxBolts)
            {
                ...
                {
                    var bolt = LightningBolt.LightningBoltEffect.BoltPool[MyUtils.GetRandomInt(0, LightningBolt.LightningBoltEffect.BoltPool.Count)];
                    var boltItem = new LightningBolt.LightningBoltInstance(bolt, null, 60 + MyUtils.GetRandomInt(-10, 15));
            ...
            for (int s = 0; s < m_activeBolts.Count; s++)
            {
                ...
                for (int r = startPos + 1; r < maxPos; r++)
                {
                    ...
                    float pulse = Extensions.MathExtensions.TrianglePulse((float)activeItem.Position, 1f, activeItem.MaxPosition / 2f) + 0.2f;
                    if (activeItem.Position < 10)
                        pulse = Extensions.MathExtensions.TrianglePulse((float)activeItem.Position, 1f, 2.5f);
