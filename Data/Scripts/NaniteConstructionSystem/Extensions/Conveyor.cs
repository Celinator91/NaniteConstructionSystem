...
namespace Ntech.Extensions
{
    public static class Conveyor
    {
        ...
        public static void RebuildConveyorList(HashSet<IMyEntity> entities)
        {
            ...
            {
                NaniteConstructionSystem.Logging.Instance.WriteLine(string.Format("REBUILD Busy.  Last Rebuild: {0}s", (DateTime.Now - m_lastRebuild).TotalSeconds));
                return;
            }
            ...
            {
                NaniteConstructionSystem.Logging.Instance.WriteLine(String.Format("RebuildConveyorList: {0}", ex.ToString()));
            }
            finally
            {
                m_busyLock.ReleaseExclusive();
                NaniteConstructionSystem.Logging.Instance.WriteLine(string.Format("REBUILD Inventory: {0}ms", (DateTime.Now - start).TotalMilliseconds));
            }
        }
