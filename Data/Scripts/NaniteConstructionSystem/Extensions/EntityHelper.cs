...
namespace Ntech.Extensions
{
    ...
    public static class GridHelper
    {
        public static List<IMyCubeGrid> GetGridGroup(IMyCubeGrid grid)
        {
            ...
            {
                NaniteConstructionSystem.Logging.Instance.WriteLine(string.Format("GetGridGroup Error: {0}", ex.ToString()));
            }
        ...
        public static bool FindFreeCargo(MyCubeBlock startBlock, MyObjectBuilder_Base item, int count, bool order = true)
        {
            ...
            if (list == null)
            {
                NaniteConstructionSystem.Logging.Instance.WriteLine(string.Format("Conveyor list is null!"));
                return false;
            }
            ...
            foreach (var inventoryItem in modifiedList)
            {
                ...
                if(targetInventory.CanItemsBeAdded(count, item.GetId()))
                {
                    ...
                    if(ownerName != null)
                        NaniteConstructionSystem.Logging.Instance.WriteLine(string.Format("TRANSFER Adding {0} {1} to {2}", count, item.GetId().SubtypeName, ownerName.CustomName));
